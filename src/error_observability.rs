...
impl ErrorTelemetry for TableError {
    fn classification(&self) -> ErrorClassification {
        match self {
            ...
            TableError::InvalidLevel { .. } => classification("table", "validation", "invalid_level", ErrorSeverity::Error),
            TableError::SStableIdExists { .. } => classification("table", "consistency", "sstable_id_exists", ErrorSeverity::Error),
            TableError::SStableIdNotFound { .. } => classification("table", "not_found", "sstable_id_not_found", ErrorSeverity::Warning),
            TableError::ManifestError { .. } => classification("table", "manifest", "manifest_error", ErrorSeverity::Error),
            TableError::InvalidOperationState { .. } => classification("table", "state", "invalid_operation_state", ErrorSeverity::Error),
            TableError::SerializationError { .. } => classification("table", "encoding", "serialization_error", ErrorSeverity::Error),
