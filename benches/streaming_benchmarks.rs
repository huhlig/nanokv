    ...
    for size in [1024, 10 * 1024, 100 * 1024].iter() {
        ...
        group.bench_with_input(BenchmarkId::from_parameter(size), size, |b, &size| {
            b.iter(|| {
                ...
                let table = PagedBTree::new(ObjectId::from(1), "bench_table".to_string(), pager).unwrap();
    ...
    for size in [1024, 10 * 1024, 100 * 1024].iter() {
        ...
        group.bench_with_input(BenchmarkId::from_parameter(size), size, |b, &size| {
            ...
            b.iter(|| {
                ...
                let value = reader.get(b"bench_key", LogSequenceNumber::from(100)).unwrap();
                black_box(value);
            });
        });
    }
...
fn bench_btree_get_stream(c: &mut Criterion) {
    ...
    for size in [1024, 10 * 1024, 100 * 1024].iter() {
        ...
        group.bench_with_input(BenchmarkId::from_parameter(size), size, |b, &size| {
            ...
            b.iter(|| {
                let reader = table.reader(LogSequenceNumber::from(100)).unwrap();
                let mut stream_opt = reader.get_stream(b"bench_key", LogSequenceNumber::from(100)).unwrap();
