...
fn test_get_stream_from_memtable() {
    ...
    // Read using stream
    let reader = lsm.reader(LogSequenceNumber::from(100)).unwrap();
    let mut stream_opt = reader.get_stream(b"stream_key", LogSequenceNumber::from(100)).unwrap();
...
fn test_mvcc_with_streaming_values() {
    ...
    let value_old = reader_old.get(b"mvcc_key", LogSequenceNumber::from(15)).unwrap().unwrap();
    assert_eq!(value_old.0, data1);
    
    let reader_new = lsm.reader(LogSequenceNumber::from(25)).unwrap();
    let value_new = reader_new.get(b"mvcc_key", LogSequenceNumber::from(25)).unwrap().unwrap();
...
fn test_delete_streaming_value() {
    ...
    // Verify it exists
    let reader = lsm.reader(LogSequenceNumber::from(100)).unwrap();
    let value = reader.get(b"delete_key", LogSequenceNumber::from(100)).unwrap();
    assert!(value.is_some());
    ...
    let value2 = reader2.get(b"delete_key", LogSequenceNumber::from(200)).unwrap();
...
fn test_overwrite_streaming_value() {
    ...
    let value = reader.get(b"overwrite_key", LogSequenceNumber::from(200)).unwrap().unwrap();
...
fn test_empty_stream() {
    ...
    let reader = lsm.reader(LogSequenceNumber::from(100)).unwrap();
    let value = reader.get(b"empty_key", LogSequenceNumber::from(100)).unwrap();
    if let Some(v) = value {
        assert_eq!(v.0.len(), 0);
    }
}
