...
fn test_filesystem_basic_operations<FS: FileSystem>(fs: &FS) {
    ...
    assert!(!fs.is_directory(path).unwrap(), "Path should not be a directory");
    ...
    assert!(!fs.exists(path).unwrap(), "File should not exist after removal");
}
...
fn test_filesystem_offset_operations<FS: FileSystem>(fs: &FS) {
    ...
    let mut buf = [0u8; 3];
    file.read_at_offset(5, &mut buf).expect("Failed to read at offset");
    assert_eq!(&buf, b"567");
    ...
    file.seek(SeekFrom::Start(0)).expect("Failed to seek");
    let cursor_before = file.stream_position().unwrap();
    file.write_to_offset(3, b"ABC").expect("Failed to write at offset");
    let cursor_after = file.stream_position().unwrap();
    assert_eq!(cursor_before, cursor_after, "Cursor should not change");
...
fn test_filesystem_directory_operations<FS: FileSystem>(fs: &FS) {
    ...
    // Create directory
    fs.create_directory(dir_path).expect("Failed to create directory");
    assert!(fs.exists(dir_path).unwrap(), "Directory should exist");
    assert!(fs.is_directory(dir_path).unwrap(), "Path should be a directory");
    ...
    // Remove directory
    fs.remove_directory(dir_path).expect("Failed to remove directory");
    assert!(!fs.exists(dir_path).unwrap(), "Directory should not exist after removal");
}
...
fn test_filesystem_nested_directories<FS: FileSystem>(fs: &FS) {
    ...
    fs.create_directory_all(nested_path).expect("Failed to create nested directories");
    assert!(fs.exists(nested_path).unwrap(), "Nested directory should exist");
    assert!(fs.is_directory(nested_path).unwrap(), "Path should be a directory");
    ...
    // Clean up
    fs.remove_directory_all("/parent").expect("Failed to remove directory tree");
    assert!(!fs.exists("/parent").unwrap(), "Parent should not exist after removal");
}
...
fn test_filesystem_file_locking<FS: FileSystem>(fs: &FS) {
    ...
    file.set_lock_status(FileLockMode::Exclusive).expect("Failed to set exclusive lock");
    assert_eq!(
        file.get_lock_status().unwrap(),
    ...
    // Test unlock
    file.set_lock_status(FileLockMode::Unlocked).expect("Failed to unlock");
    assert_eq!(
        file.get_lock_status().unwrap(),
        FileLockMode::Unlocked,
